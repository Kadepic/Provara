...
**Windows** — download [`Provara-app.zip`](https://github.com/Provara-IA/Provara/releases/latest/download/Provara-app.zip) from the [Releases](https://github.com/Provara-IA/Provara/releases) page, **unzip it** anywhere and **double-click `Provara.exe`** (inside the unzipped folder). No Python, no install needed — and startup is **instant**: the app is a ready-to-run folder, so your antivirus no longer rescans everything on every launch. It opens in your browser and works **immediately** on a bundled **sample** (~1M facts) — no waiting. To unlock the **full 72M-fact base**, click **“Base complète”** in the interface: a **one-time** download (about **6 GB of free disk space**, **15–20 minutes**) that you can also skip. A loading dialog keeps you informed throughout; the app runs **windowed** (no console window) and has a **Quit** button. *(From source instead: double-click `Lancer_Provara.bat`, which needs Python installed.)*
