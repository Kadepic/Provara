...
- **It learns what it finds** (opt-in) — a fact resolved from a **structured** source (Wikidata) is **learned** locally, typed with source + date, and re-served later **without the network, even offline**, always attributed and dated ("learned from Wikidata on…"). Strict boundary: only structured facts are learned, free text stays "reported".
- **It gives an opinion, without bluffing it** — on unsettled matters, a **built** opinion: "best destination between France, Italy and Spain?" → each verified criterion votes (**Pareto**/**Condorcet**), the rule is shown, and "my #1 criterion is GDP per capita" **re-decides**; "what do you think of electric cars?" → both sides sourced + an opinion **conditional** on your criterion. Never an invented taste.
- **It composes ambiguity instead of silently picking** — "what is the size of France?" → *"size" can mean several things: area 551,695 km² **and** population — say which reading you want*; "the size of the Eiffel Tower?" → height (330 m) led with, other readings flagged. The **speech-act engine** (the trunk) holds all readings in parallel, never a silently chosen meaning.
- **It decides under uncertainty, rule displayed** — "should I take an umbrella?" → today's rain probability **reported** (Open-Meteo) × a **displayed** utility rule (soaked = 10× carrying) → advice by **expected utility**; too close → an honest *"true coin flip"*; "…I hate carrying it" **re-decides** with your weighting.
- **It hears your state without faking it** — "I'm lost" → *"it may be that you feel… — I don't feel it myself, I read you"* + a concrete handle; no more tone-deaf "Noted". And when nothing matches: *"here is what I understood (hypothesis) + what I can do"*, never gibberish.
- **It gives you the weather and the time** (opt-in) — "what's the weather in Toulouse?" → a real, **attributed** reading (Open-Meteo); with no city, it **asks** which one and the next turn **completes** it. Local time and date from the clock.
- **It reads a site you name** (opt-in) — "look at yohanfauck.fr and tell me what you think" → it **reads the page** and reports its title + a passage, **attributed and linked**; on an opinion, it quotes the page, it does not judge.
...
**Windows** — download [`Provara-app.zip`](https://github.com/Provara-IA/Provara/releases/latest/download/Provara-app.zip) from the [Releases](https://github.com/Provara-IA/Provara/releases) page, **unzip it** anywhere and **double-click `Provara.exe`** (inside the unzipped folder). *On the very first launch, Windows SmartScreen may show “Windows protected your PC” (unsigned binary — Provara is free and open source, no paid certificate): click **“More info” → “Run anyway”**, once.* No Python, no install needed — and startup is **instant**: the app is a ready-to-run folder, so your antivirus no longer rescans everything on every launch. It opens in your browser and works **immediately** on a bundled **sample** (~1M facts) — no waiting. To unlock the **full 72M-fact base**, click **“Base complète”** in the interface: a **one-time** download (about **6 GB of free disk space**, **15–20 minutes**) that you can also skip. A loading dialog keeps you informed throughout; the app runs **windowed** (no console window) and has a **Quit** button. *(From source instead: double-click `Lancer_Provara.bat`, which needs Python installed.)*
