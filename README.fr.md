...
- **Il compte, classe, filtre et compare — exactement** — « combien de pays en Afrique ? » → un décompte **exact** ; « les 5 pays les plus peuplés d'Afrique » → un tri exact avec les valeurs ; « quels pays d'Afrique ont plus de 100 millions d'habitants ? » ou « entre 10 et 50 millions ? » → un **filtre exhaustif** (seuil ou intervalle) ; « la France est-elle plus vaste que l'Espagne ? » / « plus riche ? » → une comparaison chiffrée de **superficies** ou de **PIB** (Banque mondiale) ; « la Chine est-elle plus peuplée que l'Inde ? » → une **comparaison** chiffrée (pays par population/superficie/PIB, mais aussi **sommets par altitude** et **fleuves par longueur** — la bonne mesure est choisie selon le type d'entité) ; « quel pays a pour capitale Madrid ? » / « de quel pays Tokyo est la capitale ? » → une **recherche inverse** (de la valeur vers l'entité, multi-réponses listées) ; « quelle est la différence de population entre la France et l'Allemagne ? » → l'**écart chiffré exact** (les deux valeurs et le rapport) ; « la capitale de la France, de l'Espagne et du Japon ? » → une réponse **multi-entités** (chaque valeur vérifiée séparément) ; « population totale de l'Afrique ? » → une **agrégation** exacte (somme, moyenne, médiane) ; « quel est le plus ancien entre la bataille de Marignan et celle de Verdun ? » → *Marignan (1515)*, une comparaison de **dates**. Là où un LLM énumère de mémoire (et se trompe), Provara **parcourt toute la base**.
