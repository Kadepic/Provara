...
# (5bis) INDUCTION DE RÈGLE DE SUBSTITUTION (généralisation depuis un exemple)
A.oublie(None)
A.enregistre("quel est le chef-lieu de la France ?", "quelle est la capitale de la France ?")
check(A.substitutions_apprises().get("chef lieu") == "capitale", "substitution « chef lieu -> capitale » induite")
gen = A.alias("quel est le chef-lieu du Japon ?")
check(gen is not None and "capitale" in gen and "japon" in gen, "généralise à une phrase NEUVE (Japon)")
check("chef" not in (A.alias("chef-lieu de l'Italie") or ""), "généralise (Italie) : chef-lieu remplacé")
check(A.alias("population du Japon") is None, "phrase non concernée -> pas de substitution")
check(A.nombre_appris() == 1, "les substitutions ne gonflent pas le compte d'alias de phrase")

# (6) effacement ciblé + global (RGPD) — retire l'alias ET la substitution dérivée du texte
A.oublie(None)
A.enregistre("population de l'hexagone", "population de la France")
check(A.oublie("population de l'hexagone") >= 1, "efface l'alias ciblé (et sa substitution dérivée)")
check(A.alias("population de l'hexagone") is None, "alias ciblé effacé, plus aucune substitution résiduelle")
A.enregistre("teste global un", "capitale global un")     # de quoi avoir quelque chose à effacer
n = A.oublie(None)
check(n >= 1 and A.nombre_appris() == 0 and not A.substitutions_apprises(), "efface TOUT (RGPD)")
