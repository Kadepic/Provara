...
# ══ VOCABULAIRE ÉLARGI : bit-à-bit (frontière mesurée) + ppcm ═══════════════════════════════════════════
import math as _math
for nom, f in [("xor", lambda a, b: a ^ b), ("et_bit", lambda a, b: a & b), ("ou_bit", lambda a, b: a | b)]:
    ps = P(f, PAIRS + [(12, 10), (5, 7)])
    v = IM.examine_cible_multi(nom, ps[:5], ps[5:])
    check(f"bit-à-bit : {nom} (frontière mesurée) -> INVENTION correcte",
          v.statut == MI.INVENTION and _reproduit2(v.par, ps))
check("bit-à-bit : la réalisation xor dépend des DEUX arguments (a^b ≠ a, ≠ b)",
      IM.examine_cible_multi("x", P(lambda a, b: a ^ b, PAIRS + [(12, 10)])[:5],
                             P(lambda a, b: a ^ b, PAIRS + [(12, 10)])[5:]).par is not None)
pp = P(lambda a, b: _math.lcm(a, b), PAIRS + [(12, 10), (6, 9)])
check("ppcm : lcm(a,b) couvert par le registre binaire élargi",
      IM.examine_cible_multi("ppcm", pp[:5], pp[5:]).statut == MI.EXISTE_DEJA)
