...
- **It challenges you for real** (`_cap_challenge`, verified quiz): "challenge me on geography" → Provara **asks a question drawn from its verified base** ("What is the capital of X?"), remembers the expected answer, and **judges yours against the real fact** ("✔ Correct — Beijing" / "✘ No — the verified answer is Saint John's"). FAUX=0: both the question and the correction come from the verified base. The reverse mode is always offered (you assert, it rules True/False/Undecidable with proof). "stop" ends the challenge; a genuine new request is never held hostage.
- **It reasons inside YOUR premises** (`_cap_syllogisme`): "if all mammals nurse and the cat is a mammal, what can we deduce?" → "**From YOUR premises (Barbara/modus ponens): the cat nurses**" — a flagged hypothetical mode (it reasons in the world you posit, without asserting it as a Provara fact). If its base corroborates a premise (cat → mammal), it says so; a middle term that doesn't tie → the syllogism is refused and explained, never forced.
- **It helps you invent without fabricating** (`_cap_invention`, `_cap_creer_ouvert`, `besoin`): "how to cool a room without an air conditioner?" → it **decomposes the need** into real objective + physical channels + Carnot limit (never an idea out of thin air); a physical need outside the catalog ("preserve food without a fridge") → it **amplifies** (quantify the objective, dominant channel, free sink) instead of bluffing; "give me an idea" → it redirects to a concrete need. Speech acts are not treated as physics.
...
- **Maths, conversions and geometry in conversation** (`fonction_nl.resout_math`/`resout_conversion`, wired 2026-07-08): exact arithmetic extended to dialogue — GCD/LCM, primality, factorial, combinations/arrangements, Fibonacci, modulo ("remainder of 17 divided by 5" → 2), absolute value, cube root of perfect cubes, logarithm with explicit base (integer exponent only), rounding (French half-up)/floor (mathematical integer part, labelled)/ceiling, base conversions (binary/octal/hex ↔ decimal), modular inverse, **Roman numerals** (round-trip 1..3999 pinned; non-canonical "IIII" → abstention), **applied percentages** (discount/increase/share — the computation SHOWN: "64 (80 − 20 % = 80 − 16)"), **simple/compound interest**, **trigonometry** (degrees), **complex numbers** (modulus/argument/conjugate), **classic Nash equilibria** (`jeux_appliques`: prisoner's dilemma, battle of the sexes, matching pennies), **propositional logic** and fallacies, **geometry** (`geometrie2d`/`geometrie3d`: circle area/circumference, square, rectangle, triangle — base×height or 3 sides with the triangle inequality —, rhombus by diagonals, Pythagorean hypotenuse, cube and sphere volume), **unit conversions** (length/mass/time/volume/speed: "90 km/h in m/s" → 25, knots included), **weighted mean** (value/coefficient pairs — never the plain mean of all numbers). Any non-exact result → abstention; every route carries an anti-false-positive guard ("urban area", "prime minister", "the 3-metre ceiling" trigger nothing).
- **Everyday applied maths** (2026-07-08 extensions, same guards): prime factorization ("60 = 2² × 3 × 5"), perfect numbers, divisors, **fractions** (simplify, ↔ percentage, ← decimal, compare exactly), **inverse percentage** and pre-discount price, **rule of three**, **fair share** ("€20 among 3" → 6 each + remainder), decimal arithmetic with explicit operator, **direct comparison** of two numbers, **extremum** and **sorting** of a list, **rounding to a place value**, cylinder/cone volume, trapezoid and equilateral-triangle area, square diagonal, **conventions** (month rank, next/previous day, next letter, time divisions, degrees in a circle, polygon angle sum), **countdown to a date** ("how many days until Christmas?") and **date arithmetic** (weekday, day difference). Every route stays FAUX=0: abstention outside its scope, systematic anti-false-positive guard.
- **Date arithmetic and text operations** (`_cap_quotidien`, `_cap_texte`, 2026-07-08): "how many days between January 1st and March 15th" → 73 (exact calendar computation; missing year → clock year LABELLED, leap years included); "what day will it be in 45 days / 10 days ago" → exact date (machine clock + calendar offset); "in 3 months" (ambiguous duration) → abstention; **historical weekday** ("14 July 1789 was a Tuesday"; before 1583 → stated abstention, Julian calendar); **leap years** (Gregorian rule: 2024 yes, 2100 no); **time in a major city** ("what time is it in New York?" → IANA timezone, closed table of ~50 cities; unknown city → abstention, never the local time served in its place); **age from a birth year** (honest range). Counting letters or words, spelling, reversing, upper/lower case, sorting numbers, testing two anagrams — native exact operations, FAUX=0 by construction. **Numbers in French words** ("écris 1984 en lettres", traditional orthography stated, pinned anchors) and **numeric verifications** (even/odd, divisible, multiple, perfect square, fractions compared exactly). **Extended yes/no verification**: "est-ce que Tokyo est la capitale du Japon ?" → "Oui."; negations ("…n'est pas Lyon, si ?") get either the safe positive confirmation ("Si — Paris est bien…") or the verified fact explicitly framed — never a bare "Non" (multi-valued).
- **Elementary probability, quantitative chemistry and series** (waves 23-25, 2026-07-08): **die/coin probability** ("probability of rolling a 6 with a die" → "1/6 (≈ 16.67 %) — assuming a fair 6-sided die": the fairness assumption is always stated; N-sided dice; impossible face → "0 — a 6-sided die cannot roll a 7"; several dice → abstention); named **binomial coefficient** (interpretation shown "C(5, 2) = 10"; k > n → 0 explained), **permutations** ("how many ways to arrange 4 books" → 24), **ordinal prime** ("the 100th prime" → 541, exact enumeration), **series sums** ("sum of integers from 1 to 100" → 5050, formula shown; "of the first 5 primes" → 28). **Chemistry**: molar mass ("H2O" → 18.015 g/mol), atoms of a named element ("oxygen atoms in CO2" → 2; absent element → 0 explained; outside the reference table → abstention, never the total in its place), mass percentage, **molarity** ("2 moles in 4 litres" → 0.5 mol/L), **binary compound name** ("CO2" → carbon dioxide), **moles ↔ grams** ("36 g of water" → 1.9983 mol, n = m/M). **Tips** ("15% on €80" → tip AND total shown), **honest variable durations** (hours in a month: 672-744; days in a century: 36,524 or 36,525), Roman ↔ number both ways ("MMXXVI as a number" → 2026), "how many inhabitants in France?" → formatted answer. Thieves killed along the way (FAUX=0): "how many seconds in a year" no longer serves 1939 (a work's publication year!) — a "how many" question can never hook a year/date relation again.
- **Everyday physics, money and calendar** (waves 26-30, 2026-07-08): **kinematics** v=d/t, t=d/v (minute rounding STATED), d=v×t, compact formats ("100 km en 1h30" → 66.67 km/h); **BMI** (kg/m²), **pH** (−log₁₀), **VAT/tips** (amount AND total shown), fuel consumption per 100 km, price per kilo/litre and total price, **change counting** ("Impossible" stated if the purchase exceeds), exact coins/banknotes sums (the Fermi estimator that multiplied everything is guarded), "20 euros minus 7.50" → 12.5; **series/parallel resistors**, pendulum period (g stated), pressure F/S; **weekday past AND future** ("December 25 falls on a Friday", clock year labelled when missing), composite durations ("1 year and 6 months" → 18 months), dozens, half-litre/quarter-hour, degrees↔radians, mph, scientific notation, harmonic/geometric means (0 → honest refusal), scores ("15 out of 20" → 75 %), ordinal letters, regular polygons, solids' edges/vertices, constants e and golden ratio, South-American city times (Rio…). Each wave killed its thieves (FAUX=0): local time is never served for an unknown city, "how many" never hooks a publication year, stats never average distance and duration.
- **Advanced text, theoretical edges and number robustness** (waves 31-36, 2026-07-08): palindromes ("radar" — the object card used to answer off-target), letter occurrences, word sorting, initials, letter replacement, longest word, Morse both ways ("SOS en morse", decoding no longer stolen by the animal's definition); honest conjugation (request verb excluded, non-present tenses stated); exact age from a full birth date, future years ("what year in 10 years"), future time ("in 3 hours"), ephemerides guard (sunset ≠ clock), season with hemispheres, ISO week, 200th day of the year, relative leap years; edge cases stated (÷0, modulo 0, √negative with complex roots, √2 irrational with labelled approximation, "largest prime" → Euclid); **space-separated thousands** ("5 000" broke conversions, percentages and arithmetic — fixed at all three stages); gallon/pint → US/UK ambiguity stated, non-standard cup stated, nautical mile/light-year/carat exact, speed of sound with conditions; **continuations over calculations** ("masse molaire de H2O" then "et de CO2 ?" — formula case preserved).
