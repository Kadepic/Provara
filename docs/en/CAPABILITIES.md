...
- **Date arithmetic and text operations** (`_cap_quotidien`, `_cap_texte`, 2026-07-08): "how many days between January 1st and March 15th" → 73 (exact calendar computation; missing year → clock year LABELLED, leap years included); "what day will it be in 45 days / 10 days ago" → exact date (machine clock + calendar offset); "in 3 months" (ambiguous duration) → abstention. Counting a word's letters, spelling, reversing, testing two anagrams — native exact operations, FAUX=0 by construction.
