...
```bash
python3 _nonreg.py --jobs 8      # attendu sur la base complète : 689/689
```

À la fin, elle imprime une ligne de synthèse et rend un code de sortie :

```
=== NON-RÉG : 689/689 PASS (... via cache) en Xs ===
```
...
La gate protège **689 validateurs actifs** (`valide_*.py`) : c'est la gate de référence, **689/689**. Ce nombre correspond exactement aux fichiers `valide_*.py` présents dans le dossier `tests/` (moins `valide_commun.py`, qui est un module de *helpers* partagés explicitement exclu — ce n'est pas un validateur), plus le validateur d'interface `interface/valide_interface.py`.

Sur le disque, les **689 fichiers `valide_*.py`** vivent dans le dossier `tests/` (il n'y a pas de dossier d'archive séparé).
...
```bash
python3 tests/suite_conversation.py     # attendu : 25/25 gates au vert
```
