...
- **Arithmétique de dates et opérations textuelles** (`_cap_quotidien`, `_cap_texte`, 2026-07-08) : « combien de jours entre le 1er janvier et le 15 mars » → 73 (calcul calendaire exact ; année absente → année de l'horloge ÉTIQUETÉE, bissextile comprise) ; « quel jour serons-nous dans 45 jours / il y a 10 jours » → date exacte (horloge machine + décalage calendaire) ; « dans 3 mois » (durée ambiguë) → abstention. Compter les lettres d'un mot, épeler, écrire à l'envers, tester deux anagrammes — opérations natives exactes, FAUX=0 par construction.
