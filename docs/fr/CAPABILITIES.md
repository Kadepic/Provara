...
- **Décision quotidienne sous incertitude** (`_conseil_parapluie`, `meteo.pluie_aujourdhui`, `decision`) : « dois-je prendre un parapluie ? » → probabilité de pluie du jour **rapportée** (Open-Meteo, structurée) × règle d'utilité **affichée** (tremper = 10× le port) → conseil par **utilité espérée**, abstention honnête si l'écart est trop mince (« vrai pile ou face »), re-tranchable si ta pondération diffère. Toujours classé SUPPOSITION (conseil conditionnel), jamais fait.
...
- **Il te challenge pour de vrai** (`_cap_challenge`, quiz vérifié) : « challenge-moi sur la géographie » → Provara **pose une question tirée de sa base vérifiée** (« Quelle est la capitale de X ? »), retient la réponse attendue, et **tranche ta réponse contre le fait réel** (« ✔ Exact — Pékin » / « ✘ Non — la réponse vérifiée est Saint John's »). FAUX=0 : la question ET la correction sortent du vérifié. Mode inverse toujours offert (tu affirmes, il tranche V/F/Indécidable avec preuve). « stop » clôt le défi ; une vraie nouvelle demande n'est jamais prise en otage.
- **Il raisonne dans TES prémisses** (`_cap_syllogisme`) : « si tous les mammifères allaitent et que le chat est un mammifère, que peut-on en déduire ? » → « **D'après TES prémisses (Barbara/modus ponens) : le chat allaite** » — mode hypothétique balisé (il raisonne dans le monde que tu poses, sans l'affirmer comme un fait Provara). Si sa base corrobore une prémisse (chat → mammifère), il le dit ; un moyen terme qui ne se noue pas → syllogisme refusé et expliqué, jamais forcé.
- **Il aide à inventer sans rien fabriquer** (`_cap_invention`, `_cap_creer_ouvert`, `besoin`) : « comment rafraîchir une pièce sans climatiseur ? » → il **décompose le besoin** en objectif réel + canaux physiques + limite de Carnot (jamais une idée sortie du chapeau) ; un besoin physique hors catalogue (« conserver sans frigo ») → il **amplifie** (chiffrer l'objectif, canal dominant, puits gratuit) au lieu de bluffer ; « donne-moi une idée » → il redirige vers un besoin concret. Les actes de langage ne sont pas traités comme de la physique.
...
- **Registre des sources vérifiées** (`sources`, `datasets/sources/registry.jsonl`) : 32 sources officielles ou structurées typées par AUTORITÉ (INSEE, Légifrance, OMS, Banque mondiale, Eurostat, PubChem, NASA JPL, GBIF…), chacune avec domaines, relations alimentées et limites (rate-limit, clé, collaboratif). Frontière FAUX=0 à la source : structuré → peut nourrir un fait vérifié/appris ; consultation → rapporté attribué seulement. « D'où viennent tes informations ? » liste le registre réel ; `ou_apprendre(domaine)` dit où l'IA irait apprendre.
- **Apprentissage des faits web** (`faits_appris`) : quand Provara résout un fait sur une source **structurée** (Wikidata), il l'**apprend** — rangé en local (`~/.verax`), typé source + date — et le ressert ensuite **sans réseau, même Internet coupé**, toujours attribué et daté (« appris de Wikidata le 2026-07-06 »). Frontière FAUX=0 stricte : **seul le structuré est appris** ; le texte libre (Wikipédia) reste « rapporté », jamais appris. Un fait resservi est un instantané daté, pas une vérité intemporelle. **TTL de fraîcheur** (90 j, réglable) : un fait périmé est re-cherché d'abord quand le web est actif (ré-appris frais), et reste servi daté hors-ligne. Le diagnostic affiche le nombre de faits appris.
- **Tronc de compréhension — Phase 1** (`tronc`) : le moteur d'**actes de parole** de la spec (`SPEC_TRONC_COMPREHENSION.md`) — chaque message est classé dans une **carte fermée de 11 actes** (fait, calcul, raisonnement, avis, créer, méta, social, état, quotidien, agir, inconnu), entités/relation/régime extraits UNE fois, candidats **tenus en parallèle** (jamais un sens choisi en silence). Deux effets visibles : le **repli honnête intent-aware** — quand rien ne matche, « voici ce que j'ai compris (hypothèse) + ce que je sais faire », jamais de garbage — et l'**attunement** — « je suis perdu » reçoit une lecture d'état en **supposition** (« il se peut que tu… », jamais « je ressens »), plus jamais « C'est noté ». Périphérie gzip-kNN (distance de compression, zéro entraînement) qui **propose** une intention voisine à confiance bornée ; le cœur reste 100 % explicite. **Phase 2 — le compositeur (§10)** : l'ambiguïté se COMPOSE au lieu d'être choisie en silence — « quelle est la taille de la France ? » → les lectures vérifiées servies ensemble (superficie 551 695 km² **et** population) + l'invitation à préciser ; « la taille de la tour Eiffel » → la hauteur (330 m) menée en tête, les autres lectures signalées. Garde homonyme (un pays n'a pas de « hauteur » — le paquebot France ne répond plus pour le pays). Gate `valide_tronc` 72/72.
- **Maths, conversions et géométrie en conversation** (`fonction_nl.resout_math`/`resout_conversion`, câblage 2026-07-08) : arithmétique exacte étendue au dialogue — PGCD/PPCM, primalité, factorielle, combinaisons/arrangements, Fibonacci, modulo (« reste de 17 divisé par 5 » → 2), valeur absolue, racine cubique de cube parfait, logarithme en base explicite (exposant entier seulement), arrondi (demi-supérieur français)/partie entière (plancher mathématique, étiqueté)/plafond, conversions de base (binaire/octal/hexa ↔ décimal), inverse modulaire, **chiffres romains** (round-trip 1..3999 épinglé ; « IIII » non canonique → abstention), **pourcentages appliqués** (réduction/augmentation/part — le calcul MONTRÉ : « 64 (80 − 20 % = 80 − 16) »), **intérêts simples/composés**, **trigonométrie** (degrés), **nombres complexes** (module/argument/conjugué), **équilibres de Nash classiques** (`jeux_appliques` : dilemme du prisonnier, bataille des sexes, matching pennies), **logique propositionnelle** et sophismes, **géométrie** (`geometrie2d`/`geometrie3d` : aire/circonférence du cercle, carré, rectangle, triangle — base×hauteur ou 3 côtés avec inégalité triangulaire —, losange par diagonales, hypoténuse de Pythagore, volume du cube et de la sphère), **conversions d'unités** (longueur/masse/temps/volume/vitesse : « 90 km/h en m/s » → 25, nœuds compris), **moyenne pondérée** (paires valeur/coefficient — jamais la moyenne simple de tous les nombres). Tout résultat non exact → abstention ; chaque route porte une garde anti-faux-positif (« aire urbaine », « premier ministre », « le plafond de 3 mètres » ne déclenchent rien).
- **Maths appliquées du quotidien** (extensions 2026-07-08, mêmes garde-fous) : décomposition en facteurs premiers (« 60 = 2² × 3 × 5 »), nombre parfait, diviseurs, **fractions** (simplifier, ↔ pourcentage, ← décimal, comparer exactement), **pourcentage inverse** et prix avant réduction, **règle de trois**, **partage équitable** (« 20 € entre 3 » → 6 chacun + reste), arithmétique décimale à opérateur explicite, **comparaison directe** de deux nombres, **extremum** et **tri** d'une liste, **arrondi à un rang** (dizaine/centaine/millier), volumes cylindre/cône, aire du trapèze et du triangle équilatéral, diagonale du carré, **conventions** (rang des mois, jour suivant/précédent, lettre suivante de l'alphabet, divisions du temps, degrés du cercle, somme des angles d'un polygone), **compte à rebours** vers une date (« dans combien de jours Noël ? ») et **arithmétique de dates** (jour de la semaine, différence de jours). Chaque route reste FAUX=0 : abstention hors périmètre, garde anti-faux-positif systématique.
- **Arithmétique de dates et opérations textuelles** (`_cap_quotidien`, `_cap_texte`, 2026-07-08) : « combien de jours entre le 1er janvier et le 15 mars » → 73 (calcul calendaire exact ; année absente → année de l'horloge ÉTIQUETÉE, bissextile comprise) ; « quel jour serons-nous dans 45 jours / il y a 10 jours » → date exacte (horloge machine + décalage calendaire) ; « dans 3 mois » (durée ambiguë) → abstention ; **jour de la semaine historique** (« le 14 juillet 1789 était un mardi » ; avant 1583 → abstention dite, calendrier julien) ; **année bissextile** (règle grégorienne : 2024 oui, 2100 non) ; **heure d'une grande ville** (« quelle heure est-il à New York ? » → fuseau IANA, table fermée ~50 villes ; ville inconnue → abstention, jamais l'heure locale servie à sa place) ; **âge depuis une année de naissance** (fourchette honnête). Compter les lettres ou les mots, épeler, écrire à l'envers, majuscules/minuscules, trier des nombres, tester deux anagrammes — opérations natives exactes, FAUX=0 par construction. **Nombre en toutes lettres** (« écris 1984 en lettres », orthographe traditionnelle dite, ancres épinglées) et **vérifications numériques** (pair/impair, divisible, multiple, carré parfait, fractions comparées exactement). **Vérification oui/non étendue** : « est-ce que Tokyo est la capitale du Japon ? » → « Oui. » ; les négations (« …n'est pas Lyon, si ? ») reçoivent soit la confirmation positive sûre (« Si — Paris est bien… »), soit le fait vérifié explicitement cadré — jamais un « Non » sec (multi-valué).
- **Probabilité élémentaire, chimie quantitative et séries** (vagues 23-25, 2026-07-08) : **probabilité dé/pièce** (« probabilité d'obtenir un 6 avec un dé » → « 1/6 (≈ 16.67 %) — en supposant un dé équilibré à 6 faces » : l'hypothèse d'équilibre est toujours énoncée ; dé à N faces ; face impossible → « 0 — un dé à 6 faces ne peut pas donner 7 » ; plusieurs dés → abstention) ; **coefficient binomial** nommé (interprétation montrée « C(5, 2) = 10 » ; k > n → 0 expliqué), **permutations** (« de combien de façons ranger 4 livres » → 24), **nombre premier ordinal** (« le 100e nombre premier » → 541, énumération exacte), **sommes de séries** (« somme des entiers de 1 à 100 » → 5050, formule montrée ; « des 5 premiers nombres premiers » → 28). **Chimie** : masse molaire (« H2O » → 18.015 g/mol), atomes d'un élément nommé (« atomes d'oxygène dans CO2 » → 2 ; élément absent → 0 expliqué ; hors référentiel → abstention, jamais le total à la place), pourcentage massique, **molarité** (« 2 moles dans 4 litres » → 0.5 mol/L), **nom du composé binaire** (« CO2 » → dioxyde de carbone), **moles ↔ grammes** (« 36 g d'eau » → 1.9983 mol, n = m/M). **Pourboire** (« 15% sur 80 € » → pourboire ET total montrés), **durées variables honnêtes** (heures d'un mois : 672-744 ; jours d'un siècle : 36 524 ou 36 525), romain ↔ nombre dans les deux sens (« MMXXVI en nombre » → 2026), « combien d'habitants en France ? » → réponse formatée (« Population de la France : 68 720 337 habitants. »). Voleurs tués au passage (FAUX=0) : « combien de secondes dans une année » ne sert plus 1939 (l'année de publication d'une œuvre !) — une question « combien » n'accroche plus jamais une relation d'année/date.
- **Physique, argent et calendrier du quotidien** (vagues 26-30, 2026-07-08) : **cinématique** v=d/t, t=d/v (arrondi à la minute DIT), d=v×t, formats compacts (« 100 km en 1h30 » → 66.67 km/h) ; **IMC** (kg/m², formats 1m75/175 cm), **pH** (−log₁₀), **TVA/pourboire** (montant ET total montrés), consommation aux 100 km, prix au kilo/litre et prix total (« 3 baguettes à 1,20 € » → 3.6), **rendu de monnaie** (« Impossible » dit si l'achat dépasse), somme pièces/billets exacte (« 3 pièces de 2 € et 2 billets de 5 » → 16, l'estimateur de Fermi qui multipliait tout est gardé), « 20 euros moins 7,50 » → 12.5 ; **résistances série/parallèle**, période du pendule (g dit), pression F/S ; **jour de la semaine passé ET futur** (« le 25 décembre tombera un vendredi », année d'horloge étiquetée si absente), durées composées (« 1 an et 6 mois » → 18 mois), douzaines, demi-litre/quart d'heure, degrés↔radians, mph, notation scientifique, moyennes harmonique/géométrique (0 → refus honnête), scores (« 15 sur 20 » → 75 %), lettres ordinales (alphabet et mot), polygones réguliers, arêtes/sommets des solides, constantes e et nombre d'or, heure des villes d'Amérique du Sud (Rio…). Chaque vague a tué ses voleurs (FAUX=0) : l'heure LOCALE n'est plus jamais servie pour une ville inconnue, « combien » n'accroche plus une année de publication, les stats ne moyennent plus distance et durée.
- **Texte avancé, bords théoriques et robustesse des nombres** (vagues 31-36, 2026-07-08) : palindromes (« radar » — la fiche objet répondait à côté), occurrences d'une lettre, tri de mots, initiales, remplacement de lettre, plus long mot, morse dans les deux sens (« SOS en morse », le décodage n'est plus volé par la définition de l'animal) ; conjugaison honnête (verbe-requête exclu, temps hors présent dit) ; âge exact avec date complète, années futures (« quelle année dans 10 ans »), heure future (« dans 3 heures »), garde éphémérides (coucher du soleil ≠ horloge), saison avec hémisphères, semaine ISO, 200e jour de l'année, bissextile relative ; cas limites dits (÷0, modulo 0, √négatif avec racines complexes, √2 irrationnel avec approximation étiquetée, « plus grand premier » → Euclide) ; **milliers à espace** (« 5 000 » cassait conversions, pourcentages et arithmétique — corrigé aux trois étages) ; gallon/pinte → ambiguïté US/UK dite, tasse non normalisée dite, mile nautique/année-lumière/carat exacts, vitesse du son avec conditions ; **continuations sur les calculs** (« masse molaire de H2O » puis « et de CO2 ? » — la casse des formules est préservée).
- **Registre de capacités auto-prouvé** (`capacites`) : 281 capacités de raisonnement (causalité, lois physiques, simulation, Pareto, Condorcet, révision de croyances, lexique…) portent chacune une **preuve à réponse connue** exécutée en direct — la commande « diagnostic » affiche « capacités prouvées à l'instant : 281/281 ». Un gate d'atteignabilité (`valide_cablage`) garantit que **tout module construit est câblé au produit** (zéro orphelin) : un module non branché met la suite au rouge.
